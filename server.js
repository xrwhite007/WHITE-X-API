...
<head>
  ...
  <style>
    ...
    .is-leader { border-color: #00ffcc; background: rgba(0, 255, 204, 0.05); }
    ...
    .num-circle { display: inline-block; width: 22px; height: 22px; line-height: 22px; border-radius: 50%; font-weight: bold; }
...
<body>
  ...
  <div class="master-signal"><div id="masterResult">WAITING</div><div id="masterColor">COLOR SUGGESTION: --</div></div>
  <div class="status-card"><div id="timerDisplay">00:60</div><div id="currentPeriod">FETCHING DATA...</div></div>
  <div class="method-grid">
    <div id="box_m1" class="predict-box"><h3>Math 1</h3><div id="v1" class="pred-val">--</div></div>
    <div id="box_m2" class="predict-box"><h3>Math 2</h3><div id="v2" class="pred-val">--</div></div>
    <div id="box_m3" class="predict-box"><h3>Math 3</h3><div id="v3" class="pred-val">--</div></div>
  </div>
  <div class="history-section">
    <table><thead><tr><th>Period</th><th>Num</th><th>BS</th><th>Result</th></tr></thead><tbody id="resultBody"></tbody></table>
  </div>
<script>
const API = 'https://draw.ar-lottery02.com/WinGo/WinGo_1M/GetHistoryIssuePage.json';
let stats = { m1_w:0, m1_l:0, m2_w:0, m2_l:0, m3_w:0, m3_l:0 };
let preds = {};
let lastP = "";
let leader = "m1";
...
function updateUI(list) {
    ...
    if (!preds[next]) {
        preds[next] = { m1: lastS, m2: (lastN % 2 === 0 ? "BIG" : "SMALL"), m3: (Math.random() > 0.5 ? "BIG" : "SMALL") };
    }

    document.getElementById("v1").innerText = preds[next].m1;
    document.getElementById("v2").innerText = preds[next].m2;
    document.getElementById("v3").innerText = preds[next].m3;
    document.getElementById("masterResult").innerText = preds[next][leader];
    ...
    list.slice(0, 10).forEach(item => {
        ...
        tbody.innerHTML += '<tr><td>'+item.issueNumber.slice(-4)+'</td><td>'+n+'</td><td>'+s+'</td><td>-</td></tr>';
    });
}
